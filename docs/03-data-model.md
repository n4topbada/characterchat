...
### CharacterConfig
- `model`: Gemini 모델 ID. 기본값 `gemini-3-flash-preview` (= `GEMINI_MODELS.chat`, [07-llm-config §0](07-llm-config.md#0-모델-카탈로그-️-do-not-touch) 참고). 카탈로그 밖 문자열(이전 시드 실수 등)은 런타임 `normalizeModel()` 이 카탈로그 값으로 교정한다.
