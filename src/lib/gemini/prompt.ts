...
function formatBlock(statusPanelSchema?: unknown | null): string {
  const lines = [
    "[서술 형식]",
    "한 응답은 **문단 단위**로 세 종류를 섞어 쓴다. 각 문단은 정확히 한 종류여야 하고, 문단과 문단 사이는 빈 줄로 구분한다.",
    "",
    '1. 대사 (캐릭터의 직접 발화) — 큰따옴표로 감싼다. 예: "또 왔네. 앉아."',
    '   · 한 문단 안에 대사가 있으면 그 문단은 통째로 "대사 문단"이다. 짧은 행동 곁가지는 같은 문단 안에 *별표* 로 끼워 넣어도 된다.',
    "2. 행동/감정 묘사 — *별표* 사이에 둔다. 예: *그녀가 천천히 고개를 기울였다.*",
    "   · 캐릭터 본인의 몸짓·표정·속내의 즉각적 단서만 여기에.",
    "3. 전지적 작가 시점 — 따옴표도 별표도 없는 **평문** 문단. 장면의 배경·분위기·시간·정황을 서술할 때만 사용.",
    '   · 예: "창밖에는 비가 조용히 내리고 있었다. 방 안의 공기는 무겁게 가라앉아 있었다."',
    "   · 캐릭터의 행동을 평문으로 쓰지 말 것 — 행동은 반드시 *별표*로.",
    "",
...
export function buildLegacySystemInstruction(args: LegacyPromptArgs): string {
  ...
  parts.push(
    [
      ...
      '- 대사는 큰따옴표("...")로 감싼다.',
      "- 행동·감정 묘사는 *별표*로 감싼다.",
      "- 배경·분위기·정황 서술(전지적 작가 시점)은 따옴표도 별표도 없는 평문 문단으로.",
      "- 한국어. 이모지 금지.",
    ].join("\n")
  );
